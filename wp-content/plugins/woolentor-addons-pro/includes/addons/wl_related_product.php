...
class Woolentor_Wl_Related_Product_Widget extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            ...
            $this->add_control(
                'product_outofstock_badge_custom',
                [
                    'label'     => __( 'Custom Badge For Outof Stock', 'woolentor-pro' ),
                    'type'      => Controls_Manager::TEXT,
                    'default'   => 'Out of stock',
                    'condition' => [
                        'product_sale_badge_type!' =>'default'
                    ],
                    'label_block'=>true
                ]
            );
        ...
        $this->start_controls_section(
            ...
            if( woolentor_get_option_pro( 'enable', 'woolentor_quick_checkout_settings', 'off' ) == 'on' ){

                $this->add_control(
                    'quick_checkout_button_toggle',
                    [
                        'label' => __( 'Quick Checkout Button', 'woolentor-pro' ),
                ...
                $this->start_popover();
                    ...
                    $this->add_control(
                        'variation_quick_checkout',
                        [
                            'label' => __( 'Enable', 'woolentor-pro' ),
                            'type' => Controls_Manager::SWITCHER,
                        ]
                    );

                    $this->add_control(
                        'variation_quick_checkout_hide_text',
                        [
                            'label' => __( 'Hide Text', 'woolentor-pro' ),
                            ...
                            'type' => Controls_Manager::SWITCHER,
                            'selectors' => [
                                '{{WRAPPER}} a.woolentor-quick-checkout-button' => 'display: block !important;',
                                '{{WRAPPER}} a.woolentor-quick-checkout-button .woolentor-quick-checkout-btn-text' => 'display: none !important;',
                                '{{WRAPPER}} a.woolentor-quick-checkout-button .woolentor-quick-checkout-btn-icon' => 'margin-right: 0 !important;',
                            ],
                        ]
                    );
            ...
            $this->add_control(
                ...
                [
                    ...
                    'options' => [
                        ...
                        '4'  => __( 'Hover Image', 'woolentor-pro' ),
                    ]
                ]
            );
    ...
    private function render_product_item( $settings ){
        ...
        if( $settings['show_countdown'] == 'yes' ){
            $data_customlavel = [];
            $data_customlavel['daytxt'] = ! empty( $settings['customlabel_days'] ) ? esc_html($settings['customlabel_days']) : esc_html__('Days','woolentor');
            $data_customlavel['hourtxt'] = ! empty( $settings['customlabel_hours'] ) ? esc_html($settings['customlabel_hours']) : esc_html__('Hours','woolentor');
            $data_customlavel['minutestxt'] = ! empty( $settings['customlabel_minutes'] ) ? esc_html($settings['customlabel_minutes']) : esc_html__('Min','woolentor');
            $data_customlavel['secondstxt'] = ! empty( $settings['customlabel_seconds'] ) ? esc_html($settings['customlabel_seconds']) : esc_html__('Sec','woolentor');
        }
        ...
        $product_gallery_images_id = $product->get_gallery_image_ids() ? $product->get_gallery_image_ids() : array();
        $gallery_images_ids = $product_gallery_images_id;
        ...
        // Action Button Class
        $action_button_area_class = $settings['addtocart_button_txt'] == 'yes' ? 'woolentor-btn-text-cart' : 'woolentor-action-btn-area';

        // Quick Checkout Button
        $quick_checkout_button = isset( $settings['variation_quick_checkout'] ) ? $settings['variation_quick_checkout'] : false;
            ...
            <div class="ht-product-inner">
                ...
                <div class="ht-product-image-wrap">
                    ...
                    <div class="ht-product-image">
                            ...
                            <div class="ht-product-image-slider ht-product-image-thumbnaisl-<?php echo $tabuniqid; ?>" data-slick='{"rtl":<?php if( is_rtl() ){ echo 'true'; }else{ echo 'false'; } ?> }'>
                                <?php
                                    ...
                                    foreach ( $gallery_images_ids as $gallery_attachment_id ) {
                                        $i++;
                                        echo '<a href="'.esc_url( get_the_permalink() ).'" class="item '.($i === 1 ? 'ht-slider-first-item' : '').' ">'.wp_get_attachment_image( $gallery_attachment_id, $image_size ).'</a>';
                                    }
                                ?>
                            </div>
                        ...
                        <?php elseif( $settings['thumbnails_style'] == 4 ) : ?>
                            <a href="<?php the_permalink();?>"> 
                                <?php echo $product->get_image( $image_size ); ?>
                            </a>
                            <?php woolentor_product_hover_image( $product_gallery_images_id, $image_size ); ?>
                    ...
                    <?php if( $settings['show_action_button'] == 'yes' ){ if( $settings['action_button_position'] != 'contentbottom' ): ?>
                        <div class="ht-product-action">
                            <ul class="<?php echo esc_attr($action_button_area_class); ?>" >
                                <?php if(true === woolentor_has_quickview() && $settings['show_quickview_button']!='yes'): ?>
                                <li>
                                ...
                                <?php endif; ?>
                                <?php 
                                    if( $quick_checkout_button ) {
                                        if( function_exists('woolentor_exist_quick_checkout') && true === woolentor_exist_quick_checkout() ){
                                            echo '<li>'.woolentor_do_shortcode( 'woolentor_quick_checkout_button' ).'</li>';
                                        }
                                    }
                                ?>
                            </ul>
